...
if [ "$COMMAND" = "TestFile" ]; then
  ...
  echo spark-submit --class $CLASS --master yarn-client $JARNAME $6 $8
  nohup spark-submit --class $CLASS --master yarn-client $JARNAME $6 $8 > "$COMMAND""_krb.log" &
elif [ "$COMMAND" = "Oozie" ]; then
  CLASS=com.gmcc.hdp.demo.oozie.OozieKrbDemo
  echo yarn jar $JARNAME $CLASS $8 ${10} $4
  nohup yarn jar $JARNAME $CLASS $8 ${10} $4 > "$COMMAND""_krb.log" &
