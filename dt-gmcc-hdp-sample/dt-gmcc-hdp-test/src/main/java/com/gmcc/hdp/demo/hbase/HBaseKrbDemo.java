...
public class HBaseKrbDemo {
    ...
    public HBaseKrbDemo() {
        ...
        //添加hbase集群的master在kerberos中的principal
        hbase_config.set("hbase.master.kerberos.principal", HDPSampleConfiguration.HBASE_MASTER_KERBEROS_PRINCIPAL);
        //添加hbase集群的regionserver在kerberos中的principal
        hbase_config.set("hbase.regionserver.kerberos.principal", HDPSampleConfiguration.HBASE_REGIONSERVER_KERBEROS_PRINCIPAL);
        /**
         * kerbos认证时需要添加的配置项
         */
    ...
    public void initTable(String tableName) {
        ...
        try {
            ...
            CsvFileUtil cvs = new CsvFileUtil(HDPSampleConfiguration.LOCAL_TMP_2G_USER_FILE);
...
//                System.out.println("第" + count + "行数据添加成功");
                if (count%5000 == 0) {
                    System.out.print(".");
                }
                count++;
