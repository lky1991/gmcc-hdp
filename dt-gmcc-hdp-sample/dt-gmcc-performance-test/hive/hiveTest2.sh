...
select count(*) from $2;
select city_name_ch,count(*) from $2 group by city_name_ch;
select min(la_code),max(bscad00),count(*) from $2 group by city_name_ch;
select city_name_ch,count(*)as total from $2 group by city_name_ch order by  total;
